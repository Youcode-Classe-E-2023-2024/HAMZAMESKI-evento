...
{{--    </div>--}}
    <main class="h-full w-full flex">
        <section class="flex-grow h-full sm:py-12 dark:text-gray-100">
        ...
        <section class="" style="border-left: solid gray 0.1px">
            <p class="font-bold text-white bg-black p-2 px-4 border-b-[1px] border-solid border-pink-500">
                categories
            </p>
            </p>
            <div class="h-[94%] w-full overflow-auto flex flex-col" style="background-color: rgba(0, 0, 0, 0.86);">
                <a href="{{ route('main') }}" class="px-4 py-1 text-gray-300 hover:bg-pink-500 hover:text-black">All</a>
                @foreach($categories as $category)
                    <a href="{{ route('category-events', $category->id) }}" class="px-4 py-1 text-gray-300 hover:bg-pink-500 hover:text-black"> {{ $category->name }}</a>
                @endforeach
            </div>
        </section>
    </main>
